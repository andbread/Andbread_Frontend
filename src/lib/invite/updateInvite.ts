...
export const updateAcceptInvite = async (
  ...
  if (error) {
    ...
    captureAppError(error, {
      ...
      tags: { receiverId, senderId: senderId ?? undefined },
    })
    return null
  }
  ...
  if (!error) {
    ...
    if (error) {
      ...
      captureAppError(error, {
        action: 'invite.accept_payment_date_select',
        tags: { receiverId, senderId: senderId ?? undefined },
      })
      return
    }
  ...
  if (!error) {
    const { data, error } = await supabase
      .from('participant')
      .insert([
        { nbread_id: senderId, user_id: receiverId, is_leader: 'FALSE' },
      ])

    if (error) {
      console.error('엔빵 추가 에러 : ', error)
      captureAppError(error, {
        action: 'invite.accept_participant_insert',
        tags: { receiverId, senderId: senderId ?? undefined },
      })
      return null
    }
