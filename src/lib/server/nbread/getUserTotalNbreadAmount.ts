...
export const getUserTotalNbreadAmount = async (
  ...
  const { data: paidRecords, error: paidError } = await client
    .from('nbread_records')
    .select('nbread_id')
    .eq('user_id', userId)
    .eq('is_paid', true)
  ...
  return nbreads.reduce((sum, nbread) => {
    const individualShare = Math.floor(
      nbread.amount / Math.max(nbread.participant_count, 1),
    )
